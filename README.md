...
| 模式 | 说明 | 适用 |
|---|---|---|
| **`tui`** | 全屏仪表盘（类 btop），实时面板 + 表格 + 快捷键 | 交互终端 |
| **`repl`** | 指令式 REPL（类 Claude Code / Codex），滚动输出 + 底部 `/` 命令 | 喜欢命令驱动 |
| **`web`** | 浏览器仪表盘（Twitch 配色的拟物风 UI），HTTP + WebSocket | **默认**，Docker / 服务器，多设备访问 |
...
- 启动时：`--mode tui|repl|web|headless` 覆盖偏好；未设偏好时默认 `web`（Docker 友好，无需 TTY）；终端模式在无 TTY 时降级为 headless（web 不降级）。
...
main.py
  └─ tdm_cli/bootstrap.py    submodule 加入 sys.path、stub GUI 依赖、注入 gui、路径覆盖
     └─ tdm_cli/gui.py       CLI 版 GUIManager（接口与上游一致），更新 state + 调用前端
        ...
        └─ tdm_cli/tui/  TextualFrontend + 全屏仪表盘（交互终端）
```
...
./run.sh                 # = uv run main.py，按保存的模式启动（默认 web，见下方 Docker）
```
...
| `--mode tui\|repl\|web\|headless` | 界面模式（覆盖保存的偏好；默认读 `tdm-cli.json`，无则 `web`） |
