...
本仓库把它改造成**交互式终端程序**，提供三种可随时切换的界面模式，
...
| **`headless`** | 纯日志行，无交互 | 服务器 / Docker / 管道（无 TTY 时自动） |

模式偏好持久化在 `tdm-cli.json`，可随时切换：
- **TUI 模式**：按 `s` 打开设置，改「Interface mode」，Esc 保存即**运行中热切换**（挖矿不中断）。
- **REPL 模式**：输入 `/switch-mode tui`（或 `repl` / `headless`）即热切换。
- 启动时：`--mode tui|repl|headless` 覆盖偏好；无 TTY 一律 headless。
...
# REPL 模式
Command mode — type /help for commands, /switch-mode tui to go full-screen.
12:00:03: Claimed drop: Rust Skin
12:00:05: [watching] shroud (Rust)
tdm [Watching] › /status
12:00:12: Status   : Watching shroud
12:00:12: Drop     : Rust Skin  62.5%  ⏱ 0:42:10
tdm [Watching] › _
```
...
| `/reload` | 重新拉取库存和频道 |
| `/switch-mode tui\|repl\|headless` | 切换界面模式 |
| `/quit` | 退出 |
...
| `--mode tui\|repl\|headless` | 界面模式（覆盖保存的偏好；默认读 `tdm-cli.json`，无则 `tui`） |
